...
class RobloxAccountManager:
    ...
    def start_auto_cookie_refresh(self):
        """Auto Cookie Refresh - runs in background"""
        def refresh_loop():
            while True:
                if not self.get_secure_setting("auto_cookie_refresh", False):
                    time.sleep(60)
                    continue
                print("[Auto Refresh] Checking cookies...")
                for username, data in list(self.accounts.items()):
                    cookie = data.get("cookie")
                    if cookie:
                        RobloxAPI.keep_cookie_alive(cookie)  # keeps session alive
                time.sleep(int(self.get_secure_setting("cookie_refresh_interval", 1800)))  # default 30 min

        threading.Thread(target=refresh_loop, daemon=True, name="AutoCookieRefresh").start()
